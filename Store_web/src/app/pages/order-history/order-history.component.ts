...
export class OrderHistoryComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((data: any) => {
      ...
      if (data && data.id) {
        ...
        this.api
          .post_private('v1/orders/getOrderByIdForStore', { id: this.id })
          .then(
            (data: any) => {
              console.log(data);
              this.loaded = true;
              if (data && data.status && data.status == 200 && data.data) {
                ...
                this.datetime = moment(info.created_at).format(
                  'dddd, MMMM Do YYYY'
                );
  ...
  getDrivers() {
    ...
    this.api
      .post_private('v1/drivers/getDriversNearStore', param)
      .then(
        (data: any) => {
          ...
          if (data && data.status == 200 && data.data && data.data.length) {
            ...
            this.dummyDrivers = this.dummyDrivers.sort(
              (a, b) => a.distanceFrom - b.distanceFrom
            );
          }
        },
        (error) => {
  ...
  assignOrderToDriver(driverId: any) {
    ...
    this.util.show();
    this.api
      .post_private('v1/orders/updateFromStore', param)
      .then(
        (data: any) => {
          this.util.hide();
          console.log(data);
