...
export class InsightsComponent implements OnInit {
  ...
  getStasData() {
    this.api
      .post_private('v1/orders/getStoreStatsData', {
      ...
      .then(
        (data: any) => {
          ...
          if (data && data.status && data.status == 200 && data.data) {
                  ...
                  element.items.forEach((order: any) => {
                    ...
                    if (order && order.discount == 0) {
                      if (order.size == 1) {
                        if (
                          order &&
                          order.savedVariationsList &&
                          order.savedVariationsList.length > 0
                        ) {
                          order.savedVariationsList.forEach((variants: any) => {
                            price =
                              price +
                              parseFloat(variants.price) * variants.quantity;
                          });
                        }
                      } else {
                        price =
                          price + parseFloat(order.price) * order.quantity;
                      }
                    } else {
                      if (order.size == 1) {
                        if (
                        ...
                        price =
                          price + parseFloat(order.discount) * order.quantity;
                      }
                    }
            ...
            const todaysDateChart = [
              ...new Set(
                today.map((item: any) =>
                  moment(item.created_at).format('DD-MMM hh: a')
                )
              ),
            ];
            ...
            todaysDateChart.forEach((dt) => {
              const item = {
                date: dt,
                sells: today.filter(
                  (x: any) => moment(x.created_at).format('DD-MMM hh: a') == dt
                ),
                totalSell: 0,
              };
            ...
            weeksDateChart.forEach((dt) => {
              const item = {
                date: dt,
                sells: week.filter(
                  (x: any) => moment(x.created_at).format('DD MMM') == dt
                ),
                totalSell: 0,
              };
            ...
            this.topProducts = topProducts.sort(
              ({ counts: a }, { counts: b }) => b - a
            );
  ...
  getContent1() {
    return this.util.translate(
      'Items with images lead to more orders compared to items without them. Check your competitor item prices'
    );
  }
