  ...
  getStats() {
    ...
    if (this.from && this.to) {
      ...
      const param = {
        ...
        from: moment(this.from, 'YYYY-MM-DD HH:mm A')
          .utc(false)
          .format('YYYY-MM-DD HH:mm'),
        to: moment(this.to, 'YYYY-MM-DD HH:mm A')
          .utc(false)
          .format('YYYY-MM-DD HH:mm'),
      };
      console.log(param);
      ...
      this.storeOrder = [];
      this.api
        .post_private('v1/orders/getStoreStatsDataWithDates', param)
        .then(
          (data: any) => {
            this.apiCalled = true;
            this.util.hide();
                      ...
                      if (order && order.discount == 0) {
                        if (order.size == 1) {
                          if (
                            ...
                            order.savedVariationsList.forEach(
                              (variants: any) => {
                                price =
                                  price +
                                  parseFloat(variants.price) *
                                    variants.quantity;
                              }
                            );
                          }
