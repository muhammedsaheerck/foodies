...
class RestaurantDetailController extends GetxController
  ...
  Future<void> getStoreData() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (parser.isLoggedIn() == true) {
        ...
        log(
          "products sub index : " + productList[0].products![0].name.toString(),
        );
      }
    } else {
  ...
  void checkCartData() {
    for (var element in _productList) {
      // log("product list : " + element.products.toString());
      if (element.products != null) {
        for (var product in element.products!) {
          // log("product :" + element.products.toString());
  ...
  void onNon(bool value) {
    ...
    if (value == true) {
      for (var element in _productList) {
        element.products = element.products!.where((e) => e.veg == 0).toList();
      }
    } else {
      _productList = parser.getLocalProducts();
    }
