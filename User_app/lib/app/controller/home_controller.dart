...
class HomeController extends GetxController implements GetxService {
  ...
  Future<void> getHomeData() async {
    ...
    log(parser.getLat().toString());
    log(parser.getLng().toString());
    Response response = await parser.getHomeData(param);
    apiCalled = true;
    log("get home contr${response.statusCode}");
...
//new : The same data found in the GetHome endpoint, but the endpoint category data shows only one data, so the GetSpecific Cuisine category The List endpoint added
      Response respons = await parser.getCuisinesData();
      apiCalled = true;
      if (response.statusCode == 200) {
        Map<String, dynamic> myMap = Map<String, dynamic>.from(respons.body);
        var cuisine = myMap['data'];
        _categoryList = [];
        cuisine.forEach((data) {
          CategoryModel cuisineDates = CategoryModel.fromJson(data);
          _categoryList.add(cuisineDates);
        });
      } else {
