...
class ContactUsController extends GetxController implements GetxService {
  ...
  Future<void> saveContacts() async {
    ...
    if (response.statusCode == 200) {
      log("contact data " + response.statusCode.toString());
      log("contact as call" + response.body.toString());
  ...
  Future<void> sendToAdmin(var id) async {
    ...
    log(id.toString());
    log(parser.getSupportEmail());
    Response response = await parser.sendToMail(param);
    log("sendToMail data " + response.statusCode.toString());
    log("sendToMail as call" + response.body.toString());
    if (response.statusCode == 200) {
      isLogin.value = !isLogin.value;
      update();
      nameContact.text = '';
